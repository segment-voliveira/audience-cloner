...
Copies activations from one (audience, destination connection) pair onto another that **already
exists** — the target audience must exist and already have the destination connected. It creates
only the activations.

**This replaces by default.** Every activation already on the target destination is **deleted**
first, then the copies are created, so the target ends up mirroring the source exactly instead of
accumulating. The wizard states how many will be deleted, lists them by name and id, and the
confirmation reads `Delete 3 and create 2 activation(s) on aud_…?`. Deleting an activation stops
that data flowing to the destination, so nothing happens before you confirm — and `--dry-run` shows
the whole plan without touching anything.

| Flag | Effect |
| --- | --- |
| `--from-audience ID` / `--to-audience ID` | Skip the audience pickers. |
| `--from-connection ID` / `--to-connection ID` | Skip the destination pickers (`ii_…` ids). |
...
Because replace deletes before it creates, two things are deliberate:

- **If any delete fails, no creates run.** Otherwise the stale activation and its replacement would
  both be live, double-sending events to the destination. The run stops and reports instead.
- **If the run ends with the target short of activations** (a delete succeeded but a create failed,
  or creates were aborted), the payloads of everything deleted are printed so they can be recreated
  without digging through the Segment UI.

Replace makes the operation idempotent: re-running it deletes the copies it previously made and
recreates them, leaving the target identical to the source.
...
Guards, because this writes to third-party destinations:

- Copying onto the same audience/destination pair is refused — under replace that would delete the
  activations you are copying from.
- If source and target point at **different destinations**, it warns: `destinationMapping.actionId`
  and destination-specific settings are unlikely to be valid on the target.
...
```sh
python3 -m unittest test_payloads -v   # 22 tests, no network
```
